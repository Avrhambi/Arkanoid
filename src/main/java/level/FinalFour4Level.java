...
/**
 * The type FinalFour4Level.
 *
 * @author Avraham Bicha
 * @since 2026-09-07
 */
public class FinalFour4Level implements LevelInformation {
    ...
    private static Color[] colorsArray = new Color[]{Color.gray, Color.red, Color.yellow,
            Color.green, Color.white, Color.pink, Color.cyan};

    /**
     * Constructor.
     */
    public FinalFour4Level() {
    ...
    public List<Block> createBlocks() {
        ...
        for (int i = 0; i < NUMBER_OF_ROWS; i++) {
            for (int j = 1; j <= BLOCKS_IN_ROW; j++) {
                Block b1 = new Block(new Rectangle(new Point(GameLevel.SCREEN_WIDTH
                        - GameLevel.SCREEN_FRAME_SIZE - (BLOCK_WIDTH * j), GameLevel.SCREEN_HEIGHT
                        - FIRST_ROW_Y_OFFSET - (BLOCK_HEIGHT * i)), BLOCK_WIDTH, BLOCK_HEIGHT), colorsArray[i]);
                blockList1.add(b1);
            }
        }
        return blockList1;
    ...
    public Sprite createBackground() {
        return new Background(new Rectangle(new Point(0, GameLevel.SCREEN_HEIGHT),
                GameLevel.SCREEN_WIDTH, GameLevel.SCREEN_HEIGHT), Color.decode("#101820"));
    }
